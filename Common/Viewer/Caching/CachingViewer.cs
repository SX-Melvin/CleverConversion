...
namespace CleverConversion.Common.Viewer.Caching
{
    public class CachingViewer : IViewer
    {
        ...
        public async Task<Viewer.Entities.Pages> GetPagesAsync(FileCredentials fileCredentials, int[] pageNumbers)
        {
        ...
        public async Task<Page> GetPageAsync(FileCredentials fileCredentials, int pageNumber)
        {
            ...
            var bytes = await _fileCache.GetOrAddAsync(cacheKey, fileCredentials.FilePath, async () =>
            {
                using (await _asyncLock.LockAsync(fileCredentials.FilePath))
                {
                    return await _fileCache.GetOrAddAsync(cacheKey, fileCredentials.FilePath, async () =>
                    {
            ...
            return CreatePage(pageNumber, bytes);
        }
        private async Task<DocumentInfo> FetchAndCacheDocumentInfoAsync(FileCredentials fileCredentials)
        {
            using (await _asyncLock.LockAsync(fileCredentials.FilePath))
            {
                var cacheKey = CacheKeys.FILE_INFO_CACHE_KEY;
                var docInfo = await _viewer.GetDocumentInfoAsync(fileCredentials);
                await _fileCache.SetAsync(cacheKey, fileCredentials.FilePath, docInfo);
                return docInfo;
            }
        }
        ...
        {
            var cacheKey = CacheKeys.FILE_INFO_CACHE_KEY;
            return _fileCache.TryGetValueAsync<DocumentInfo>(cacheKey, fileCredentials.FilePath)
                ?? FetchAndCacheDocumentInfoAsync(fileCredentials);
        }

        public Task<byte[]> GetPdfAsync(FileCredentials fileCredentials)
        {
            var cacheKey = CacheKeys.PDF_FILE_CACHE_KEY;
            return _fileCache.GetOrAddAsync(cacheKey, fileCredentials.FilePath, async () =>
            {
                using (await _asyncLock.LockAsync(fileCredentials.FilePath))
                {
                    return await _fileCache.GetOrAddAsync(cacheKey, fileCredentials.FilePath, () =>
            ...
            string resourceName)
        {
            ...
            return _fileCache.GetOrAddAsync(cacheKey, fileCredentials.FilePath,
                async () =>
                {
                    using (await _asyncLock.LockAsync(fileCredentials.FilePath))
                    {
        ...
        private async Task<Viewer.Entities.Pages> CreatePages(FileCredentials fileCredentials, int[] pageNumbers)
        {
            using (await _asyncLock.LockAsync(fileCredentials.FilePath))
            {
                var pagesOrNulls = GetPagesOrNullsFromCache(fileCredentials.FilePath, pageNumbers);
        ...
        private Entities.Pages Combine(List<CachedPage> dst, Entities.Pages missing)
        {
            var pages = dst
                .Select(pageOrNull =>
                {
            ...
            return new Entities.Pages(pages);
        }

        private Task SaveToCache(string filePath, Entities.Pages createdPages)
        {
        ...
        private Entities.Pages ToPages(List<CachedPage> pagesOrNulls)
        {
            var pages = pagesOrNulls
                .Select(p => CreatePage(p.PageNumber, p.Data))
                .ToList();

            var result = new Entities.Pages(pages);
            return result;
        }
